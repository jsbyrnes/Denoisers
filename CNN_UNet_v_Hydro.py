...
print("CNN-UNetVT" + model_v + " initialized with " + str(count_parameters(model)) + " trainable parameters")
model = model.float()
...
dataloader_train = DataLoader(train_dataset, batch_size = 32, shuffle=True, num_workers=24)
...
if train:
    ...
    for epoch in range(num_epochs):
        ...
        #save current model
        torch.save(model.state_dict(), 'model_test-CNN_UNetVTHydro_fac' + str(fac) + model_v + '.pt')
    ...
    np.save('CNN_UnetVT_Hydrofac' + str(fac), np.vstack( (training_hist, val_hist) ))
