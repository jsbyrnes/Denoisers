...
if __name__ == "__main__":
    ...
    if channels == 'all':
        model = CNN_UNet_v2.UNetv1(drop = drop, ncomp = 4, fac = fac, use_skip_attention=use_skip_attention, use_bottleneck_attention=use_bottleneck_attention) #pytorch version of amanda's tensorflow UNet
    elif channels == 'vertical':
        model = CNN_UNet_v2.UNetv1(drop = drop, ncomp = 1, fac = fac, use_skip_attention=use_skip_attention, use_bottleneck_attention=use_bottleneck_attention) #pytorch version of amanda's tensorflow UNet
    elif channels == 'pressure':
        model = CNN_UNet_v2.UNetv1(drop = drop, ncomp = 2, fac = fac, use_skip_attention=use_skip_attention, use_bottleneck_attention=use_bottleneck_attention) #pytorch version of amanda's tensorflow UNet
    elif channels == 'horizontals':
        model = CNN_UNet_v2.UNetv1(drop = drop, ncomp = 2, fac = fac, use_skip_attention=use_skip_attention, use_bottleneck_attention=use_bottleneck_attention) #pytorch version of amanda's tensorflow UNet
    elif channels == '3C':
        model = CNN_UNet_v2.UNetv1(drop = drop, ncomp = 3, fac = fac, use_skip_attention=use_skip_attention, use_bottleneck_attention=use_bottleneck_attention) #pytorch version of amanda's tensorflow UNet
    ...
    with torch.no_grad():
        ...
        for idx in range(test_sweeps):
            for large_batch in dataloader_test:
                ...
                target_large, trace_clean, trace_combined = CNN_UNet_v2.calculate_signal_from_mask(
                    inputs_large, mask_large, nperseg, hop_length
                )

                out = model(inputs_large)
                denoised = CNN_UNet_v2.time_domain_output(out.cpu()).numpy()
                ...
                if channels in ["all", "3C"]:
                    ...
                    for channel in [1, 2]:  # Horizontal channels
                        ...
                        for batch in range(denoised.shape[0]):
                            y = trace_clean[batch, channel, :].numpy()
