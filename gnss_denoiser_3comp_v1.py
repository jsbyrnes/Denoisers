...
# LOAD THE DATA
print("LOADING DATA")
x_data = h5py.File('nd3_data_subset.hdf5','r')
n_data = h5py.File('729k_noise_subset.hdf5','r')
sl_data = h5py.File('softlayers_subset.hdf5','r')
...
if train:
    if not(norm_input):
        ...
        model_save_file="quickie_3comp_v1_"+str(fac)+"_"+str(datetime.datetime.today().month)+"-"+str(datetime.datetime.today().day)+".weights.h5"
    else:
        ...
        model_save_file="quickie_3comp_norm_input_v1_"+str(fac)+"_"+str(datetime.datetime.today().month)+"-"+str(datetime.datetime.today().day)+".weights.h5"
else:
    if not(norm_input):
        model_save_file="quickie_3comp_v1_"+str(fac)+"_5-14.weights.h5"
        #model_save_file="quickie_3comp_v1_"+str(fac)+"_8-18.tf"
    else:
        model_save_file="quickie_3comp_norm_input_v1_"+str(fac)+"_5-14.weights.h5"
...
if train:
    ...
    history=model.fit(x = gnss_tools.stft_3comp_data_generator('v1',batch_size,x_data[x_train_inds,:],n_data[n_train_inds,:],sr,nperseg, noverlap, norm_input),
                        steps_per_epoch=(len(x_train_inds))//batch_size,
                        validation_data=gnss_tools.stft_3comp_data_generator('v1',batch_size,x_data[x_test_inds,:],n_data[n_test_inds,:],sr,nperseg, noverlap, norm_input),
                        validation_steps=(len(x_test_inds))//batch_size,
                        epochs=epos, callbacks=[model_checkpoint_callback,csv_logger])
